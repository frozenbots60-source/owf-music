...
async def process_play_command(message: Message, query: str):
    ...
    if isinstance(result, dict) and "playlist" in result:
        ...
        if secs > MAX_DURATION_SECONDS:
            await processing_message.edit(
                "❌ Streams longer than 2 hours are not allowed. If u are the owner of this bot contact @KustXoffical to upgrade your plan"
            )
            return
...
async def fallback_local_playback(chat_id: int, message: Message, song_info: dict):
    ...
    try:
        ...
        base_caption = (
            "<blockquote>"
            "<b>OWIF ✘ Music Streaming</b> (Local Playback)\n\n"
